...
export type CloudProviderCredential = Database["public"]["Tables"]["cloud_provider_credentials"]["Row"]
...
export async function getCloudProviderById(id: number) {
  const supabase = await createClient()

  const { data, error } = await supabase.from("cloud_providers").select("*").eq("id", id).single()
...
// Get a cloud provider by slug
export async function getCloudProviderBySlug(slug: string) {
  const supabase = await createClient()

  const { data, error } = await supabase.from("cloud_providers").select("*").eq("slug", slug).single()

  if (error) {
    console.error(`Error fetching cloud provider with slug ${slug}:`, error)
    throw new Error("Failed to fetch cloud provider")
  }
...
// Get user's cloud provider credentials
export async function getUserCloudProviderCredentials() {
  const supabase = await createClient()
...
export async function getCloudProviderCredentialById(id: string) {
  const supabase = await createClient()
...
export async function deleteCloudProviderCredential(id: string) {
  const supabase = await createClient()
